...
<template>
    <section class="breathe-hero flex flex-col items-center justify-center text-center px-10 pt-32 pb-24 relative z-10">
        <!-- THE BREATHE BUTTON -->
        <button 
            @click="toggleSound"
            id="breatheBtn"
            :class="['breathe-btn relative w-[150px] h-[150px] rounded-full bg-transparent border-none cursor-pointer flex flex-col items-center justify-center mb-8 opacity-0 fill-mode-forwards', isPlaying ? 'active' : '']"
        >
            <div class="breathe-ring-3 absolute inset-[-30px] rounded-full border border-talkheals-rose/10 animate-breatheRing duration-[4s] delay-[1000ms]"></div>
            <div class="breathe-ring-2 absolute inset-[-15px] rounded-full border-[1.5px] border-talkheals-gold/22 animate-breatheRing duration-[4s] delay-[500ms]"></div>
            <div class="breathe-ring-1 absolute inset-0 rounded-full bg-talkheals-rose/18 animate-breatheRing duration-[4s]"></div>
            <div 
                class="breathe-core w-[90px] h-[90px] rounded-full flex flex-col items-center justify-center gap-1 shadow-[0_12px_40px_rgba(196,154,138,0.35)] relative z-10 transition-all duration-400"
        ...
        <div class="breathe-click-note text-[0.7rem] text-talkheals-muted/60 tracking-[0.08em] mt-[-4px] mb-8 opacity-0 fill-mode-forwards">
            {{ c.click_note }}
        </div>

        <h1 class="hero-heading font-serif text-[clamp(2.4rem,5vw,4.2rem)] font-extralight leading-[1.05] text-talkheals-deep mb-0 opacity-0 fill-mode-forwards">
